export interface ActivityItem {
  icon: string;
  data: string;
}

export interface ActivityGroup {
  title: string;
  today: ActivityItem[];
}

export const mutualFriend = [
  {
    profile: 'assets/images/user/2.jpg',
    status: 'online',
    name: 'Bucky Barnes',
    mail: 'winter@gmail.com',
  },
  {
    profile: 'assets/images/user/10.jpg',
    status: 'busy',
    name: 'Sarah Loren',
    mail: 'barnes@gmail.com',
  },
  {
    profile: 'assets/images/user/6.jpg',
    status: 'offline',
    name: 'Jason Borne',
    mail: 'jasonb@gmail.com',
  },
  {
    profile: 'assets/images/user/8.jpg',
    status: 'offline',
    name: 'Comeren Diaz',
    mail: 'comere@gmail.com',
  },
  {
    profile: 'assets/images/user/4.jpg',
    status: 'busy',
    name: 'Andew Jon',
    mail: 'andrewj@gmail.com',
  },
  {
    profile: 'assets/images/user/2.jpg',
    status: 'online',
    name: 'Johny William',
    mail: 'winter@gmail.com',
  },
  {
    profile: 'assets/images/user/2.jpg',
    status: 'online',
    name: 'Bucky Barnes',
    mail: 'winter@gmail.com',
  },
  {
    profile: 'assets/images/user/6.jpg',
    status: 'offline',
    name: 'Jason Borne',
    mail: 'jasonb@gmail.com',
  },
  {
    profile: 'assets/images/user/8.jpg',
    status: 'offline',
    name: 'Comeren Diaz',
    mail: 'comere@gmail.com',
  },
  {
    profile: 'assets/images/user/4.jpg',
    status: 'busy',
    name: 'Andew Jon',
    mail: 'andrewj@gmail.com',
  },
  {
    profile: 'assets/images/user/2.jpg',
    status: 'online',
    name: 'Johny William',
    mail: 'winter@gmail.com',
  },
  {
    profile: 'assets/images/user/2.jpg',
    status: 'online',
    name: 'Bucky Barnes',
    mail: 'winter@gmail.com',
  },
  {
    profile: 'assets/images/user/8.jpg',
    status: 'offline',
    name: 'Comeren Diaz',
    mail: 'comere@gmail.com',
  },
  {
    profile: 'assets/images/user/4.jpg',
    status: 'busy',
    name: 'Andew Jon',
    mail: 'andrewj@gmail.com',
  },
];

export const activityFeed = [
  {
    profile: 'assets/images/user/2.jpg',
    name: 'Andew Jon',
    time: '20 min Ago',
  },
  {
    profile: 'assets/images/user/3.jpg',
    name: 'Johny Waston',
    time: '1 hour Ago',
  },
  {
    profile: 'assets/images/user/6.jpg',
    name: '1 hour Ago',
    time: '20 min Ago',
  },
  {
    profile: 'assets/images/user/7.jpg',
    name: 'Sarah Loren',
    time: '2 Day Ago',
  },
  {
    profile: 'assets/images/user/9.jpg',
    name: 'Johny Waston',
    time: '5 Day Ago',
  },
  {
    profile: 'assets/images/user/10.jpg',
    name: 'Comeren Diaz',
    time: '6 Day Ago',
  },
];

export const educationData = [
  {
    id: 1,
    title: 'The New College of Design',
    date: '2001 - 2006',
    desc: 'Breaking Good, RedDevil, People of Interest, The Running Dead, Found, American Guy.',
    detailClass: false,
  },
  {
    id: 2,
    title: 'Digital Design Intern',
    date: '2006-2008',
    desc: 'Digital Design Intern for the “Multimedz” agency. Was in charge of the communication with the clients.',
    detailClass: true,
  },
  {
    id: 3,
    title: 'Rembrandt Institute',
    date: '2008',
    desc: 'Five months Digital Illustration course. Professor: Leonardo Stagg.',
    detailClass: false,
  },
  {
    id: 4,
    title: 'UI/UX Designer',
    date: '2001 - 2006',
    desc: 'Breaking Good, RedDevil, People of Interest, The Running Dead, Found, American Guy.',
    detailClass: true,
  },
  {
    id: 5,
    title: 'The Digital College',
    date: '2010',
    desc: '6 months intensive Motion Graphics course. After Effects and Premire. Professor: Donatello Urtle.',
    detailClass: false,
  },
  {
    id: 6,
    title: 'The New College of Design',
    date: '2001 - 2006',
    desc: 'Breaking Good, RedDevil, People of Interest, The Running Dead, Found, American Guy.',
    detailClass: true,
  },
];

export const HobbiesData = [
  {
    id: 1,
    title: 'Hobbies:',
    desc: 'I like to ride the bike to work, swimming, and working out. I also like reading design magazines, go to museums, and binge watching a good tv show while it’s raining outside.',
    detailClass: false,
  },
  {
    id: 2,
    title: 'Favourite Music Bands / Artists:',
    desc: 'Iron Maid, DC/AC, Megablow, The Ill, Kung Fighters, System of a Revenge.',
    detailClass: true,
  },
  {
    id: 3,
    title: 'Favourite TV Shows:',
    desc: 'Breaking Good, RedDevil, People of Interest, The Running Dead, Found, American Guy.',
    detailClass: false,
  },
  {
    id: 4,
    title: 'Favourite Books:',
    desc: 'The Crime of the Century, Egiptian Mythology 101, The Scarred Wizard, Lord of the Wings, Amongst Gods, The Oracle, A Tale of Air and Water.',
    detailClass: true,
  },
  {
    id: 5,
    title: 'Favourite Movies:',
    desc: 'Idiocratic, The Scarred Wizard and the Fire Crown, Crime Squad Ferrum Man.',
    detailClass: false,
  },
  {
    id: 6,
    title: 'Favourite Writers:',
    desc: 'Martin T. Georgeston, Jhonathan R. Token, Ivana Rowle, Alexandr Platt, Marcus Roth.',
    detailClass: true,
  },
  {
    id: 7,
    title: 'Favourite Games:',
    desc: 'The First of Us, Assassin’s Squad, Dark Assylum, NMAK16, Last Cause 4, Grand Snatch Auto.',
    detailClass: false,
  },
  {
    id: 8,
    title: 'Other Interests:',
    desc: 'Swimming, Surfing, Scuba Diving, Anime, Photography, Tattoos, Street Art.',
    detailClass: true,
  },
];

export const addFriends = [
  {
    id: 1,
    profile: 'assets/images/user/2.jpg',
    name: 'Jason Borne',
  },
  {
    id: 2,
    profile: 'assets/images/user/3.png',
    name: 'Anna Mull',
  },
  {
    id: 3,
    profile: 'assets/images/user/2.jpg',
    name: 'Karlene Lex',
  },
  {
    id: 4,
    profile: 'assets/images/user/2.jpg',
    name: 'Vella Chism',
  },
  {
    id: 5,
    profile: 'assets/images/user/2.jpg',
    name: 'Wai Schalk',
  },
  {
    id: 6,
    profile: 'assets/images/user/2.jpg',
    name: 'Jason Borne',
  },
];

export const viewedProfile = [
  {
    id: 1,
    profile: 'assets/images/user/2.jpg',
    name: 'Jason Borne',
  },
  {
    id: 2,
    profile: 'assets/images/user/3.png',
    name: 'Anna Mull',
  },
  {
    id: 3,
    profile: 'assets/images/user/2.jpg',
    name: 'Karlene Lex',
  },
  {
    id: 4,
    profile: 'assets/images/user/2.jpg',
    name: 'Vella Chism',
  },
  {
    id: 5,
    profile: 'assets/images/user/2.jpg',
    name: 'Wai Schalk',
  },
  {
    id: 6,
    profile: 'assets/images/user/2.jpg',
    name: 'Jason Borne',
  },
];

export class log {
  static todayData: ActivityGroup[] = [
    {
      title: 'Today',
      today: [
        {
          icon: 'thumbs-up',
          data: 'Karlene Lex and Comeren Diaz now friends.',
        },
        {
          icon: 'user-plus',
          data: 'Karlene Lex and Comeren Diaz now friends.',
        },
        { icon: 'message-square', data: 'Sarah Loren wrote on your timeline' },
        { icon: 'thumbs-up', data: "Johny Waston likes your post's." },
        {
          icon: 'user-plus',
          data: 'Andew Jon became friends with Comeren Diaz.',
        },
        {
          icon: 'user-plus',
          data: 'Johny Waston became friends with Bucky Barnes.',
        },
      ],
    },
    {
      title: '25 December',
      today: [
        { icon: 'thumbs-up', data: 'Comeren Diaz likes your photos.' },
        { icon: 'thumbs-up', data: "Johny Waston likes your post's." },
        {
          icon: 'user-plus',
          data: 'Karlene Lex and Comeren Diaz now friends.',
        },
        {
          icon: 'user-plus',
          data: 'Johny Waston became friends with Bucky Barnes.',
        },
        { icon: 'message-square', data: 'Sarah Loren wrote on your timeline' },
        { icon: 'message-square', data: 'Comeren Diaz wrote on your timeline' },
      ],
    },
    {
      title: '8 september',
      today: [
        {
          icon: 'thumbs-up',
          data: 'Johny Waston became friends with Bucky Barnes.',
        },
        { icon: 'thumbs-up', data: 'Sarah Loren wrote on your timeline' },
        { icon: 'user-plus', data: 'Comeren Diaz wrote on your timeline' },
        { icon: 'user-plus', data: 'Comeren Diaz likes your photos.' },
        { icon: 'message-square', data: "Johny Waston likes your post's." },
        {
          icon: 'message-square',
          data: 'Karlene Lex and Comeren Diaz now friends.',
        },
      ],
    },
    {
      title: '6 June',
      today: [
        { icon: 'thumbs-up', data: 'Comeren Diaz likes your photos.' },
        { icon: 'user-plus', data: "Johny Waston likes your post's." },
        {
          icon: 'message-square',
          data: 'Karlene Lex and Comeren Diaz now friends.',
        },
        {
          icon: 'thumbs-up',
          data: 'Johny Waston became friends with Bucky Barnes.',
        },
        { icon: 'user-plus', data: 'Sarah Loren wrote on your timeline' },
        { icon: 'user-plus', data: 'Comeren Diaz wrote on your timeline' },
      ],
    },
  ];
}

export const massages = [
  {
    id: 1,
    replyMassage: 'me',
    profile: 'assets/images/user/1.jpg',
    name: 'Jason Borne',
    time: '1 Year Ago',
    massage:
      'we are working for the dance and sing songs. this car is very awesome for the youngster. please vote this car and like our post',
  },
  {
    id: 2,
    replyMassage: 'reply',
    profile: 'assets/images/user/1.jpg',
    name: 'Alexendra Dhadio',
    time: '1 Month Ago',
    massage:
      'we are working for the dance and sing songs. this car is very awesome for the youngster. please vote this car and like our post',
  },
  {
    id: 3,
    replyMassage: 'me',
    profile: 'assets/images/user/1.jpg',
    name: 'Olivia Jon',
    time: '15 Days Ago',
    massage:
      'we are working for the dance and sing songs. this car is very awesome for the youngster. please vote this car and like our post',
  },
  {
    id: 4,
    replyMassage: 'me',
    profile: 'assets/images/user/1.jpg',
    name: 'Issa Bell',
    time: '1 Year Ago',
    massage:
      'we are working for the dance and sing songs. this car is very awesome for the youngster. please vote this car and like our post',
  },
];

export const massages2 = [
  {
    id: 1,
    replyMassage: 'me',
    profile: 'assets/images/user/1.jpg',
    name: 'Jason Borne',
    time: '1 Year Ago',
    massage:
      'we are working for the dance and sing songs. this car is very awesome for the youngster. please vote this car and like our post',
  },

  {
    id: 2,
    replyMassage: 'me',
    profile: 'assets/images/user/1.jpg',
    name: 'Olivia Jon',
    time: '15 Days Ago',
    massage:
      'we are working for the dance and sing songs. this car is very awesome for the youngster. please vote this car and like our post',
  },
];
